...
/**
 * Configuration for the manual parameter editor.
 */
export interface ManualParameterEditorConfig {
  /** Whether to show advanced parameter options */
  readonly showAdvancedOptions: boolean;
  
  /** Whether to validate parameter types */
  readonly validateTypes: boolean;
  
  /** Whether to show parameter examples */
...
/**
 * Manual parameter information entry.
 */
export interface ManualParameterEntry {
  /** Command ID this parameter belongs to */
 ...
 */
export class ManualParameterEditor {
  private panel: vscode.WebviewPanel | undefined;
  private currentCommand: CommandMetadata | undefined;
  private manualEntries: Map<string, ManualParameterEntry[]> = new Map();
  
  constructor(
    private readonly context: vscode.ExtensionContext,
    private readonly storageManager: FileStorageManager,
    private readonly config: ManualParameterEditorConfig
  ) {
   ...
   */
  public async openEditor(command: CommandMetadata): Promise<void> {
    ...
    if (this.panel) {
      ...
      this.panel = vscode.window.createWebviewPanel(
        ...
        `Edit Parameters: ${command.displayName}`,
   ...
   */
  public getManualEntries(commandId: string): ManualParameterEntry[] {
   ...
   */
  public async addOrUpdateEntry(entry: ManualParameterEntry): Promise<void> {
    const commandEntries = this.manualEntries.get(entry.commandId) || [];
    
    // Find existing entry with same parameter name
    const existingIndex = commandEntries.findIndex(
      e => e.parameter.name === entry.parameter.name
    );
   ...
   */
  public async removeEntry(commandId: string, parameterName: string): Promise<void> {
   ...
   */
  public mergeWithAutomatic(command: CommandMetadata): CommandMetadata {
    ...
    const mergedParams: ParameterInfo[] = [];
    ...
    const mergedSignature: CommandSignature = {
   ...
   */
  private async handleAddParameter(data: any): Promise<void> {
    if (!this.currentCommand) return;
    
    const entry: ManualParameterEntry = {
   ...
   */
  private async handleUpdateParameter(data: any): Promise<void> {
    if (!this.currentCommand) return;
    ...
    if (existingEntry) {
      const updatedEntry: ManualParameterEntry = {
        ...existingEntry,
        parameter: {
   ...
   */
  private async handleRemoveParameter(data: any): Promise<void> {
    if (!this.currentCommand) return;
   ...
   */
  private async loadManualEntries(): Promise<void> {
    try {
      ...
      if (data) {
        ...
        for (const [commandId, commandEntries] of Object.entries(entries)) {
          this.manualEntries.set(commandId, commandEntries as ManualParameterEntry[]);
        }
   ...
   */
  private async saveManualEntries(): Promise<void> {
    try {
      // Convert Map to object for JSON serialization
      const entries: Record<string, ManualParameterEntry[]> = {};
...
<body>
   ...
   */
  private generateParametersList(entries: ManualParameterEntry[], type: 'manual' | 'automatic'): string {
    if (entries.length === 0) {
   ...
   */
  private getEditorScript(): string {
    return `
