...
import { DocumentationExporter, ExportConfig, ExportResult, DocumentationFormat } from '../export/documentation-exporter';
...
export interface DocumentationManagerConfig {
  ...
  readonly defaultFormats: DocumentationFormat[];
  
  /** Whether to auto-export on command changes */
  ...
  readonly enableQualityAssessment: boolean;
  
  /** Version tracking enabled */
  readonly enableVersionTracking: boolean;
}
 ...
 * 
 * The DocumentationManager provides a comprehensive interface for managing
 * generated documentation with export configuration and quality assessment.
 */
export class DocumentationManager {
  private panel: vscode.WebviewPanel | undefined;
  private currentCommands: CommandMetadata[] = [];
  private currentTestResults: TestResult[] = [];
  private lastExportResult: ExportResult | undefined;
  
  constructor(
    private readonly context: vscode.ExtensionContext,
    private readonly exporter: DocumentationExporter,
    private readonly storageManager: FileStorageManager,
    private readonly config: DocumentationManagerConfig
  ) {}
 ...
 /**
  ...
  private async handleExportDocumentation(exportConfig: Partial<ExportConfig>): Promise<void> {
    try {
      ...
      const customExporter = new DocumentationExporter(exportConfig);
      
      // Export documentation
      const result = await customExporter.exportDocumentation(
        this.currentCommands,
        this.currentTestResults
      );
      ...
      if (result.success) {
        vscode.window.showInformationMessage(
          `Documentation exported successfully! Generated ${result.files.length} files.`
        );
      } else {
   ...
   */
  private async handleQualityAssessment(): Promise<void> {
   ...
   */
  private async handleOpenExportDirectory(): Promise<void> {
   ...
   */
  private async handlePreviewFormat(format: DocumentationFormat): Promise<void> {
   ...
   */
  private assessDocumentationQuality(): QualityMetrics {
...
<body>
   ...
   */
  private generateQualitySection(metrics: QualityMetrics): string {
    return `
      <div class="section">
   ...
   */
  private generateStatusSection(): string {
    return `
 ...
 /**
  ...
  private getScoreDescription(score: number): string {
    if (score >= 90) return 'Excellent documentation quality';
    if (score >= 70) return 'Good documentation quality';
    if (score >= 50) return 'Fair documentation quality - room for improvement';
