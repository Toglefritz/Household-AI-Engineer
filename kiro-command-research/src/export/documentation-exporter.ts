...
export interface ExportConfig {
  ...
  readonly includeChangeTracking: boolean;
...
export interface DocumentationMetadata {
  ...
  /** Number of test results included */
...
export interface ChangeSummary {
  /** Commands added since last version */
  readonly commandsAdded: string[];
  
  /** Commands removed since last version */
  readonly commandsRemoved: string[];
  
  /** Commands modified since last version */
  readonly commandsModified: Array<{
    commandId: string;
    changes: string[];
  }>;
  
  /** Signature changes */
  readonly signatureChanges: Array<{
    commandId: string;
    oldSignature?: string;
    newSignature?: string;
  }>;
...
export class DocumentationExporter {
  ...
  public async exportDocumentation(
    ...
    previousVersion?: DocumentationMetadata
    ...
    try {
      ...
      const metadata = await this.createDocumentationMetadata(
        commands,
        testResults,
        previousVersion
      );
  ...
  ): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportMarkdown(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
    const files: Array<{ path: string; format: DocumentationFormat; size: number }> = [];
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportHtml(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
    const files: Array<{ path: string; format: DocumentationFormat; size: number }> = [];
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportJson(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
    const files: Array<{ path: string; format: DocumentationFormat; size: number }> = [];
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportTypeScript(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
    const files: Array<{ path: string; format: DocumentationFormat; size: number }> = [];
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportOpenApi(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
    const files: Array<{ path: string; format: DocumentationFormat; size: number }> = [];
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportPdf(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportConfluence(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
   ...
   * @returns Promise that resolves to generated files
   */
  private async exportGitBook(context: TemplateContext): Promise<Array<{ path: string; format: DocumentationFormat; size: number }>> {
   ...
   */
  private generateMarkdownReadme(context: TemplateContext): string {
...
`;
   ...
   * @returns Markdown content
   */
  private generateMarkdownReference(context: TemplateContext): string {
...
`;
    ...
    for (const [category, categoryCommands] of categories) {
      ...
      const subcategories = new Map<string, CommandMetadata[]>();
   ...
   * @returns Markdown content
   */
  private generateCommandMarkdown(command: CommandMetadata): string {
   ...
   * @returns Markdown content
   */
  private generateMarkdownApi(context: TemplateContext): string {
...
`;
   ...
   * @returns Markdown content
   */
  private generateMarkdownExamples(context: TemplateContext): string {
...
`;
   ...
   */
  private generateHtmlDocumentation(context: TemplateContext): string {
...
<body>
   ...
   * @returns HTML content
   */
  private generateHtmlCommandList(commands: CommandMetadata[]): string {
...
`;
   ...
   */
  private generateTypeScriptDefinitions(context: TemplateContext): string {
...
`;
   ...
   * @returns TypeScript content
   */
  private generateTypeScriptRegistry(context: TemplateContext): string {
    const { commands } = context;
...
`;
   ...
   */
  private async createDocumentationMetadata(
    commands: CommandMetadata[],
    testResults: TestResult[],
    previousVersion?: DocumentationMetadata
    ...
    return {
      ...
      generatorVersion: '1.0.0', // Would come from package.json
   ...
   */
  private formatChangeSummary(changeSummary: ChangeSummary): string {
    let summary = '';
