...
import { TestResult, TestSession, ResultAnalysis } from './result-capture';
import { DetailedSideEffect } from './side-effect-detector';
import { ExecutionResult } from './command-executor';
...
export interface StorageConfig {
  ...
  /** Maximum number of results to keep per command */
  readonly maxResultsPerCommand: number;
  
  /** Maximum age of results to keep (in days) */
  readonly maxResultAge: number;
  
  /** Whether to compress old results */
  readonly compressOldResults: boolean;
...
export interface StorageStatistics {
  ...
  /** Number of commands with stored results */
  readonly commandsWithResults: number;
  
  /** Total storage size in bytes */
  readonly totalSizeBytes: number;
  
  /** Oldest result timestamp */
  readonly oldestResult?: Date;
  
  /** Newest result timestamp */
  readonly newestResult?: Date;
  
  /** Results by command */
  readonly resultsByCommand: Record<string, number>;
  
  /** Storage health status */
  readonly health: 'healthy' | 'warning' | 'error';
  
  /** Health issues if any */
  readonly healthIssues: string[];
}

/**
 * Search criteria for finding test results.
 */
export interface SearchCriteria {
  /** Command ID to search for */
  ...
  /** Whether result has notes */
  readonly hasNotes?: boolean;
  
  /** Side effect count range */
  readonly sideEffectRange?: {
    min: number;
    max: number;
  };
  ...
  /** Sort order */
  readonly sortBy?: 'timestamp' | 'duration' | 'commandId' | 'riskLevel';
...
export interface ExportOptions {
  /** Export format */
  readonly format: 'json' | 'csv' | 'markdown' | 'html';
  
  /** Whether to include detailed side effects */
  readonly includeSideEffects: boolean;
  
  /** Whether to include full execution results */
  readonly includeExecutionDetails: boolean;
...
export class TestResultStorage {
  ...
  private readonly resultCache: Map<string, TestResult> = new Map();
  private readonly indexCache: Map<string, string[]> = new Map(); // commandId -> resultIds
   ...
   */
  public async storeResult(result: TestResult): Promise<void> {
   ...
   */
  public async getResult(resultId: string): Promise<TestResult | undefined> {
    // Check cache first
   ...
   */
  public async getResultsForCommand(commandId: string, limit?: number): Promise<TestResult[]> {
   ...
   */
  public async searchResults(criteria: SearchCriteria): Promise<TestResult[]> {
  ...
  public async getStatistics(): Promise<StorageStatistics> {
    ...
    try {
      ...
      let oldestResult: Date | undefined;
      let newestResult: Date | undefined;
      const resultsByCommand: Record<string, number> = {};
      ...
      const health: StorageStatistics['health'] = 
        healthIssues.length === 0 ? 'healthy' :
   ...
   */
  public async exportResults(criteria: SearchCriteria, options: ExportOptions): Promise<string> {
  ...
  public async cleanup(): Promise<number> {
    ...
    try {
      ...
      const resultsByCommand: Record<string, TestResult[]> = {};
      for (const result of allResults) {
   ...
   */
  public async restoreFromBackup(backupPath: string, overwrite = false): Promise<number> {
   ...
   */
  private async writeResultToFile(result: TestResult): Promise<void> {
   ...
   * @returns Promise that resolves to test result or undefined
   */
  private async loadResultFromFile(resultId: string): Promise<TestResult | undefined> {
   ...
   * @returns File path
   */
  private getCommandIndexPath(commandId: string): string {
   ...
   */
  private async updateCommandIndex(commandId: string): Promise<void> {
   ...
   */
  private async loadCommandIndex(commandId: string): Promise<void> {
    const indexPath = this.getCommandIndexPath(commandId);
   ...
   */
  private applyFilters(results: TestResult[], criteria: SearchCriteria): TestResult[] {
   ...
   */
  private applySorting(results: TestResult[], criteria: SearchCriteria): TestResult[] {
   ...
   */
  private async cleanupOldResults(commandId: string): Promise<void> {
   ...
   * @returns JSON string
   */
  private exportToJson(results: TestResult[], options: ExportOptions): string {
   ...
   * @returns CSV string
   */
  private exportToCsv(results: TestResult[], options: ExportOptions): string {
    const headers = [
      'ID',
      'Command ID',
      'Success',
   ...
   */
  private exportToMarkdown(results: TestResult[], options: ExportOptions): string {
   ...
   * @returns HTML string
   */
  private exportToHtml(results: TestResult[], options: ExportOptions): string {
    let html = `
<!DOCTYPE html>
<html>
<head>
